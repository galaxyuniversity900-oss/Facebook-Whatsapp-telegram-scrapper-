...
app = FastAPI(title="Unified Social Intelligence Platform", version="1.1.0")
...
class FacebookRequest(BaseModel): page_id: str = Field(min_length=1, max_length=128); limit: int = Field(default=25, ge=1, le=100)
class TelegramRequest(BaseModel): query: str = Field(min_length=1, max_length=500); limit: int = Field(default=50, ge=1, le=100)
...
def current_user(authorization: str | None = Header(default=None)) -> str:
    ...
    try:
        payload = jwt.decode(authorization[7:], JWT_SECRET, algorithms=[ALGORITHM]); return str(payload["sub"])
...
@app.get("/health")
def health(): return {"status":"ok","service":"social-intelligence","version":"1.1.0"}
...
async def crawl(body: CrawlRequest, _: str = Depends(current_user)):
    imported=0
    async with httpx.AsyncClient(timeout=20,follow_redirects=True,headers={"User-Agent":"SocialIntel/1.1 public-data-client"}) as client:
        for url in body.urls:
            ...
            text=response.text[:100000]; external=hashlib.sha256(f"{url}|{text[:4000]}".encode()).hexdigest()
            save_record({"source":"web","external_id":external,"text":text,"url":str(url)})
            imported+=1
    ...
    try:
        ...
        try:
            async for message in client.search_global(body.query,limit=body.limit):
                ...
                save_record({"source":"telegram","external_id":f"{chat_id}:{message.id}","author":getattr(getattr(message,"from_user",None),"username",None),"text":text})
                count+=1
...
async def facebook(body: FacebookRequest, _: str = Depends(current_user)):
    ...
    endpoint=f"https://graph.facebook.com/{os.getenv('META_GRAPH_VERSION','v23.0')}/{body.page_id}/feed"
    ...
    for item in payload.get("data",[]): save_record({"source":"facebook","external_id":item.get("id"),"author":(item.get("from") or {}).get("name"),"text":item.get("message"),"url":item.get("permalink_url"),"published_at":item.get("created_time")})
    return {"imported":len(payload.get("data",[]))}

@app.get("/webhooks/whatsapp")
def whatsapp_verify(request: Request):
    if request.query_params.get("hub.mode")=="subscribe" and request.query_params.get("hub.verify_token")==os.getenv("WHATSAPP_VERIFY_TOKEN"): return int(request.query_params.get("hub.challenge") or "0")
    ...
    for entry in payload.get("entry",[]):
        for change in entry.get("changes",[]):
            for message in change.get("value",{}).get("messages",[]):
                save_record({"source":"whatsapp","external_id":message.get("id"),"author":message.get("from"),"text":message.get("text",{}).get("body")}); count+=1
