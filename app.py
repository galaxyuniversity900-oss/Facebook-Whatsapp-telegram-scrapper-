...
app = FastAPI(title="Unified Social Intelligence Platform", version="1.2.0")
...
def save_record(data: dict) -> int:
    ...
    with SessionLocal() as db:
        ...
        row = Record(source=source, external_id=external_id, author=data.get("author"), text=text[:100000], url=str(data["url"]) if data.get("url") else None, published_at=data.get("published_at"), fingerprint=fingerprint)
...
def assert_public_host(url: str) -> None:
    ...
    try:
        ...
        raise HTTPException(400, "Unable to resolve target host")
    addresses = {item[4][0] for item in infos}
    for address in addresses:
        ip = ipaddress.ip_address(address)
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved or ip.is_multicast or ip.is_unspecified:
            raise HTTPException(400, "Private or non-public network targets are not allowed")

@app.get("/health")
def health(): return {"status":"ok","service":"social-intelligence","version":"1.2.0"}

@app.post("/auth/login")
def login(body: Login):
    if not hmac.compare_digest(body.email, ADMIN_EMAIL) or not hmac.compare_digest(body.password, ADMIN_PASSWORD):
    ...
    async with httpx.AsyncClient(timeout=20,follow_redirects=False,headers={"User-Agent":"SocialIntel/1.2 public-data-client"}) as client:
        for url in body.urls:
            ...
            try:
                ...
                external=hashlib.sha256(f"{target}|{text[:4000]}".encode()).hexdigest()
                save_record({"source":"web","external_id":external,"text":text,"url":target}); imported+=1
...
def whatsapp_verify(request: Request):
    ...
    if request.query_params.get("hub.mode")=="subscribe" and expected and hmac.compare_digest(request.query_params.get("hub.verify_token", ""), expected):
        return int(request.query_params.get("hub.challenge") or "0")
    ...
    for entry in payload.get("entry",[]):
        for change in entry.get("changes",[]):
            for message in change.get("value",{}).get("messages",[]):
                if message.get("id"):
                    save_record({"source":"whatsapp","external_id":message["id"],"author":message.get("from"),"text":message.get("text",{}).get("body")}); count+=1
